"""
🍌 TEMU 电商智能作图系统 V15.2.1
完整修复版 - 修复图片显示/下载问题 + 优化标题生成 + 修复thinking_level兼容性
核心作者: 企鹅 & 小明
商业订阅: 企鹅 & Jerry
...
def get_project_output_base_dir():
    s = get_settings()
    base = (
        s.get("project_output_dir") or str(Path.home() / "Downloads" / APP_NAME)
    ).strip()
...
def render_settings_defaults_tab():
    ...
    with c2:
        ...
        project_output_dir = st.text_input(
            "项目保存目录",
            value=s.get(
                "project_output_dir", str(Path.home() / "Downloads" / APP_NAME)
            ),
...
def show_project_center():
    ...
    st.markdown("统一管理进行中任务、历史项目、回收站和本地文件。")
